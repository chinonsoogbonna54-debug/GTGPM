...
export default function Navbar({ search, onSearchChange }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-brand-line bg-brand-bg">
      <div className="max-w-[935px] mx-auto flex items-center justify-between gap-3 px-4 h-14">
  ...
  <nav className="flex items-center gap-2 sm:gap-4 flex-shrink-0 text-xs sm:text-sm">
    <Link to="/" className="font-medium whitespace-nowrap">Feed</Link>
    <Link to="/about" className="text-brand-grey whitespace-nowrap">About</Link>
    {isLoggedIn && (
      <>
        <Link to="/admin/dashboard" className="font-medium text-brand-red whitespace-nowrap">Dashboard</Link>
        <button
          onClick={() => {
            logout();
            navigate("/");
          }}
          className="text-brand-grey whitespace-nowrap"
        >
          Log out
        </button>
      </>
